...
export class EvmTestWallet {
    ...
    async topUpFromDonor(
        ...
        const donorWallet = await EvmTestWallet.fromAddress(
            donor,
            this.provider
        )
    ...
    public async signTypedData(typedData: EIP712TypedData): Promise<string> {
        return EvmTestWallet.signTypedData(this.signer, typedData)
    }
